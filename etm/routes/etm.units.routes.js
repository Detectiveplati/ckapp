...
function buildUnitPayload(body, partial = false) {
  ...
  if (!partial || body.type !== undefined) {
    if (!TYPE_DEFAULTS[type]) errors.push('Type must be freezer, chiller, warmer, or ambient');
    else update.type = type;
  }

  ['location', 'area', 'inUseComment', 'notes'].forEach((field) => {
...
router.get('/:id', async (req, res) => {
  try {
    if (!dbReady()) return res.status(503).json({ ok: false, error: 'Core MongoDB is not connected' });
